...
def apply_global_style():
    """Carica lo stile dal file esterno e gestisce l'iniezione HTML corretta"""
    css_path = "style.css"
    if os.path.exists(css_path):
        with open(css_path, "r", encoding="utf-8") as f:
            css_content = f.read()
        
        # Tutto il CSS e l'immagine HTML vanno dentro un UNICO markdown
        st.markdown(f"""
            <style>
                :root {{
            ...
            <img src="{LOGO_URL}" class="app-logo-fixed">
...
if not st.session_state.authenticated:
    apply_global_style()
    # Stile inline per nascondere la sidebar solo in login
