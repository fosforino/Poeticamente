...
def show():
    ...
    st.markdown(f"""
        ...
        div.stButton > button[key="btn_salva"] {{ background: #2e7d32 !important; box-shadow: 0 5px 0 #1b5e20, 0 8px 15px rgba(0,0,0,0.2) !important; }}
        div.stButton > button[key="btn_stampa"] {{ background: #455a64 !important; box-shadow: 0 5px 0 #263238, 0 8px 15px rgba(0,0,0,0.2) !important; }}
        div.stButton > button[key="btn_cancella"] {{ background: #8e0000 !important; box-shadow: 0 5px 0 #4a0000, 0 8px 15px rgba(0,0,0,0.2) !important; }}
        </style>
        {img_html}
    ...
    if "utente" in st.session_state:
        ...
        try:
            # Allineato a colonna 'autore' e 'created_at'
        ...
        with b1:
            if st.button("💾 Custodisci", key="btn_salva"):
                if titolo:
                    try:
                        dati = {"titolo": titolo, "versi": contenuto, "categoria": categoria, "autore": nome_poeta}
                        if opera_corrente:
